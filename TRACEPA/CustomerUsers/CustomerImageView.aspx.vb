    ...
    Private Sub imgbtnBack_Click(sender As Object, e As ImageClickEventArgs) Handles imgbtnBack.Click
        ...
        Try
            ...
            If sSelectedChecksIDs.Length > 0 Then
                dtDoc = objclsSearch.SearchDocuments(sSession.AccessCode, sSession.AccessCodeID, sSession.UserID, sSelectedCabID, sSelectedSubCabID, sSelectedFolID, sSelectedDocTypeID, sSelectedKWID, sSelectedDescID, "", "", "", "", "", sSession.UsrDeptID, sSelectedFrmtID, "", sSelectedCrByID, "")
                If dtDoc.Rows.Count > 0 Then
                    ...
                    Response.Redirect(String.Format("~/CustomerUsers/SearchDocuments.aspx?ImageViewID={0}&SelectedIndexID={1}&SelectedChecksIDs={2}&SelId={3}&Title={4}", oImageViewID, oSelectedIndexID, oSelectedChecksIDs, oSelId, HttpUtility.UrlDecode(objclsGRACeGeneral.EncryptQueryString(sTitle))), False)
                    Exit Sub
                Else
            ...
            Else
                ...
                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Popup", "swal('No documents found in this Collation','', 'info');", True)
            End If
            Response.Redirect(String.Format("~/CustomerUsers/SearchDocuments.aspx?ImageViewID={0}", oImageViewID), False)
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
