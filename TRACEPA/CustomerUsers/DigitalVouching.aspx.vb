...
Imports System.IO
Public Class DigitalVouching
    Inherits System.Web.UI.Page
    Private sFormName As String = "DigitalVouching"
    ...
    Private Sub BindCabinet()
        Try
            ddlCabinet.DataSource = objIndex.LoadCabinetBasedOnDept(sSession.AccessCode, sSession.AccessCodeID, sSession.UserID, sSession.UsrDeptID)
    ...
    Private Sub BindCustomers()
        Try
            ...
            ddlCustomer.DataSource = objclsDigitalFilingDashboard.GetCustomerDetails(sSession.AccessCode, sSession.AccessCodeID, sSession.UsrCompanyID)
            ...
            ddlCustomer.Enabled = False
            ' ddlCustomer.SelectedValue = objclsDigitalFilingDashboard.GetCustomerDetails(sSession.AccessCode, sSession.AccessCodeID, sSession.UserID)
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
    ...
    Private Sub BindAllAttachedDocuments()
        ...
        Try
            If ddlCustomer.SelectedIndex > 0 Then
                ' dt = objclsADOBatch.GetDigitalVouchingDashboard(sSession.AccessCode, sSession.AccessCodeID, ddlCustomer.SelectedValue)
                dt = objclsADOBatch.GetDigitalVouchingDashboard(sSession.AccessCode, sSession.AccessCodeID, sSession.UsrDeptID)

            Else
                dt = objclsADOBatch.GetDigitalVouchingDashboard(sSession.AccessCode, sSession.AccessCodeID, 0)
            End If
    ...
    Private Sub gvUploadedDocument_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvUploadedDocument.RowCommand
        Dim lblAtchDocID As New Label, lblDFAttachID As New Label, lblCabID As New Label
        'Dim sPaths As String, sDestFilePath As String
        Dim oAttachID As New Object
        Dim oCabID As New Object
        Try
            lblError.Text = "" : lblMsg.Text = ""
            ...
            If e.CommandName.Equals("EditRow") Then
                Dim clickedRow As GridViewRow = TryCast(DirectCast(e.CommandSource, ImageButton).NamingContainer, GridViewRow)
                lblAtchDocID = DirectCast(clickedRow.FindControl("lblAtchDocID"), Label)
                lblCabID = DirectCast(clickedRow.FindControl("lblCabId"), Label)
                iEDTPKId = (Val(lblAtchDocID.Text))
                oAttachID = HttpUtility.UrlEncode(objclsGRACeGeneral.EncryptQueryString(Val(lblAtchDocID.Text)))
                oCabID = HttpUtility.UrlEncode(objclsGRACeGeneral.EncryptQueryString(Val(lblCabID.Text)))

                Response.Redirect(String.Format("~/CustomerUsers/DigitalVouchingDetails.aspx?AttachID={0}&CbnId={1}", oAttachID, oCabID), False)
        ...
        Try
            ...
            Else
                ...
                If ddlfolder.SelectedIndex = 0 Then
                    ...
                    Exit Sub
                Else
                    iFolder = objIndex.CheckFoldersName(sSession.AccessCode, sSession.AccessCodeID, ddlfolder.SelectedItem.Text, iSubCabinet) 'Checking for Folder Existance
                    If iFolder = 0 Then
