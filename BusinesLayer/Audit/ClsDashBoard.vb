...
Public Class ClsDashBoard
    ...
    Public Function LoadReportChart1(ByVal sNameSpace As String, ByVal iACID As Integer, ByVal Isectionid As Integer, ByVal Imonth As Integer, ByVal iCustId As Integer, ByVal Iyearid As Integer) As DataTable
        ...
        dtDisplay.Columns.Add("SubSectionName")
        ...
        Try
            ...
            sSql = "" : sSql = "select CA_PKID from CRPA_AuditAssest "
            sSql = sSql & " where CA_AsgNo=(select AUD_ID from Audit_Schedule"
            sSql = sSql & " where AUD_YearID=19 and AUD_MonthID=" & Imonth & " and AUD_KitchenID=" & iCustId & " and AUD_SectionID=5)"
            IAuditid = objDBL.SQLExecuteScalar(sNameSpace, sSql)

            sSql = "" : sSql = "select CASU_SUBSECTIONNAME,MIN(b.CRAD_Results) as Result,b.CRAD_SUBSECTIONID from crpa_subsection"
            sSql = sSql & " left join CRPA_ChecklistAuditAssest b on b.CRAD_SUBSECTIONID=CASU_ID and b.CRAD_CAuditID=" & IAuditid & " and b.CRAD_MonthID=" & Imonth & " and b.CRAD_Locationid=" & iCustId & ""
            ...
            For i = 0 To dt.Rows.Count - 1
                dRow = dtDisplay.NewRow
                dRow("SrNo") = i + 1
                If IsDBNull(dt.Rows(i)("CASU_SUBSECTIONNAME")) = False Then
                    dRow("SubSectionName") = dt.Rows(i)("CASU_SUBSECTIONNAME")
                Else
                    dRow("SubSectionName") = 0
                End If
    ...
    Public Function LoadReportChart2(ByVal sNameSpace As String, ByVal iACID As Integer, ByVal Isectionid As Integer, ByVal Imonth As Integer, ByVal iCustId As Integer, ByVal Iyearid As Integer) As DataTable
        ...
        Try
            ...
            sSql = "Select CA_PKID from CRPA_AuditAssest As a"
            sSql = sSql & " left Join audit_schedule b On b.aud_id=a.CA_AsgNo"
            sSql = sSql & " where CA_FinancialYear=" & Iyearid & " And CA_SECTIONID=" & Isectionid & " And CA_LOCATIONID=" & iCustId & " And AUD_MonthID=" & Imonth & ""
            iCAID = objDBL.SQLExecuteScalarInt(sNameSpace, sSql)

            sSql = "" : sSql = "Select b.cvr_id, b.cvr_name as Ratingname, count(a.crad_findings) As TotalCount from CRPA_ChecklistAuditAssest a"
            sSql = sSql & " join crpa_valuerating b On a.crad_sectionid = b.cvr_auditid  And b.cvr_id=a.crad_findings"
            sSql = sSql & " join audit_schedule c On c.aud_sectionid = a.CRAD_SECTIONID"
    ...
    Public Function LoadAuditorRatingNames(ByVal sNameSpace As String, ByVal iACID As Integer, ByVal Isectionid As Integer, ByVal Imonth As Integer, ByVal Iyearid As Integer) As DataTable
        ...
        Try
            sSql = "Select cvr_id, cvr_name as Rating_Name from crpa_valuerating"
            sSql = sSql & " where cvr_auditid= " & Isectionid & " And CVR_YearID =" & Iyearid & "And CVR_FLAG ='A' group by cvr_id,cvr_name"
            dt = objDBL.SQLExecuteDataTable(sNameSpace, sSql)
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    dRow = dtDisplay.NewRow
                    If IsDBNull(dt.Rows(i)("cvr_id")) = False Then
                        dRow("Id") = Val(dt.Rows(i)("cvr_id")) * 10
                    End If
    ...
    Public Function GetColorAndRange(ByVal sAC As String, ByVal iACID As Integer, ByVal iYearID As Integer, ByVal iValueID As Integer, ByVal iSecID As Integer) As DataTable
        ...
        Try
            sSql = "Select CRAT_Name,CRAT_Color From CRPA_Rating Where  CRAT_FLAG='A' And CRAT_STARTVALUE<=round(" & iValueID & ",1) AND CRAT_ENDVALUE>=round(" & iValueID & ",1) And CRAT_YEARID=" & iYearID & " And CRAT_COMPID=" & iACID & ""
            If iSecID > 0 Then
