...
Public Class clsIssueFollowupTracker
    ...
    Public Function LoadAuditCode(ByVal sAC As String, ByVal iACID As Integer, ByVal iYearId As Integer) As DataTable
        Dim sSql As String
        Dim dt As New DataTable
        Try
            sSql = "select AUD_id, AUD_Code from Audit_Schedule where AUD_yearid=" & iYearId & " and AUD_operation='Submitted' and AUD_compid=" & iACID & "  order by AUD_ID"
    ...
    Public Function GetCustomerName(ByVal sAC As String, ByVal iACID As Integer, ByVal iAUDCodeID As Integer) As Integer
        ...
        Try
            sSql = "select AUD_KitchenID from Audit_Schedule where AUD_ID=" & iAUDCodeID & " and AUD_CompID=" & iACID & " "
        ...
        End Try
    End Function
    Public Function LoadExistingIssue(ByVal sNameSpace As String, ByVal iACID As Integer, ByVal iYearID As Integer, ByVal iAuditID As Integer, ByVal iIssueStatus As Integer) As DataTable
        ...
        Try
            sSql = "select CIR_StartRange,CIR_EndRange from CAIQ_IssueRange where  CIR_FLAG='A' and CIR_CompId=" & iACID & ""
            ...
            sSql = sSql & " a.CRAD_Results, a.CRAD_COMMENTS,j.CID_PKID,j.CID_IssueStatus  from CRPA_ChecklistAuditAssest a "
            ...
            sSql = sSql & " left join CAIQ_IssueTracker_details j on j.CID_AuditCodeID=a.CRAD_CAuditID and j.CID_ChecklistID=a.CRAD_PKID"
            sSql = sSql & " Left Join CRPA_AuditAssest k on k.CA_PKID=a.CRAD_CAuditID "
            sSql = sSql & " where CRAD_Results between " & dt.Rows(0)("CIR_StartRange") & " and " & dt.Rows(0)("CIR_EndRange") & " and k.CA_FinancialYear=" & iYearID & " and CASP_CompId ='" & iACID & "' "
            If iAuditID > 0 Then
                sSql = sSql & " and CRAD_CAuditID ='" & iAuditID & "'"
            End If
    ...
    '        sSql = sSql & " d.cap_id as processID,d.CAP_PROCESSNAME as Processname,e.CASP_ID as SubProcessID,e.CASP_SUBPROCESSNAME as SubProcess, f.CF_PKID as FactorID, f.CF_Name as FactorName, "
    '        sSql = sSql & " g.CFC_PKID as FCategoryID, g.CFC_Name as FCategoryName, h.CD_PKID as DescriptorsID, h.CD_Name as DescriptorName, i.CCD_PKID as DescriptionID, i.CCD_Name as DescName, "
    '        sSql = sSql & " a.CRAD_Results, a.CRAD_COMMENTS from CRPA_ChecklistAuditAssest a "
    '        sSql = sSql & " Left Join CRPA_Section b on b.CAS_Id=a.CRAD_SECTIONID "
    '        sSql = sSql & " Left Join CRPA_SubSection c on c.CASU_Id=a.CRAD_SUBSECTIONID"
    '        sSql = sSql & " Left Join CRPA_Process d on d.CAP_Id=a.CRAD_ProcessID"
    '        sSql = sSql & " Left Join CRPA_SubProcess e on e.CASP_ID=a.CRAD_SUBPROCESSID"
    ...
    Public Function GetAuditCode(ByVal sAC As String, ByVal iACID As Integer, ByVal iAuditID As Integer) As DataTable
        ...
        Dim dt As New DataTable
        Try
            sSql = "select * from Audit_Schedule where AUD_ID=" & iAuditID & "  and AUD_CompID=" & iACID & ""
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
    ...
    Public Function LoadAllIssueTrackerJobCode(ByVal sAC As String, ByVal iACID As Integer, ByVal iYearID As Integer, ByVal iAuditID As Integer) As DataTable
        Dim sSql As String
        Try
            sSql = "Select CID_PKID,CID_IssueJobNo From CAIQ_IssueTracker_details Where CID_AuditCodeID=" & iAuditID & " and  CID_YearID=" & iYearID & "  And CID_CompID=" & iACID & " Order by CID_PKID Desc"
    ...
    Public Function GetMaxID(ByVal sAC As String, ByVal iACID As Integer) As Integer
        Dim sSql As String
        Try
            sSql = "Select Count(*)+1 from CAIQ_IssueTracker_details where CID_CompID=" & iACID & ""
    ...
    Public Sub SubmitIssueTracker(ByVal sAC As String, ByVal iACID As Integer, ByVal iYearID As Integer, ByVal iUserID As Integer, ByVal iITID As Integer)
        Dim sSql As String
        Try
            sSql = "Update CAIQ_IssueTracker_details Set CID_Status='Submitted',CID_SubmittedBy=" & iUserID & ",CID_SubmittedOn=GetDate() Where CID_PKID=" & iITID & "  And CID_CompID=" & iACID & ""
            objDBL.SQLExecuteNonQuery(sAC, sSql)
        Catch ex As Exception
            Throw
        End Try
    End Sub
    Public Function SaveIssueTracker(ByVal sAC As String, ByVal objFCIDDetails As clsIssueFollowupTracker)
        ...
        Try
            ...
            ObjParam(iParamCount) = New OleDb.OleDbParameter("@CID_FactorsID", OleDb.OleDbType.Integer, 4)
            ObjParam(iParamCount).Value = objFCIDDetails.iCID_FactorsID
            ObjParam(iParamCount).Direction = ParameterDirection.Input
            iParamCount += 1

            ObjParam(iParamCount) = New OleDb.OleDbParameter("@CID_DescriptorID", OleDb.OleDbType.Integer, 4)
            ObjParam(iParamCount).Value = objFCIDDetails.iCID_DescriptorID
            ObjParam(iParamCount).Direction = ParameterDirection.Input
            iParamCount += 1

            ObjParam(iParamCount) = New OleDb.OleDbParameter("@CID_DescriptionID", OleDb.OleDbType.Integer, 4)
            ObjParam(iParamCount).Value = objFCIDDetails.iCID_DescriptionID
            ObjParam(iParamCount).Direction = ParameterDirection.Input
            iParamCount += 1

            ObjParam(iParamCount) = New OleDb.OleDbParameter("@CID_Results", OleDb.OleDbType.Integer, 4)
            ObjParam(iParamCount).Value = objFCIDDetails.iCID_Results
            ObjParam(iParamCount).Direction = ParameterDirection.Input
            iParamCount += 1
            ...
            Arr = objDBL.ExecuteSPForInsertARR(sAC, "spCAIQ_IssueTracker_details", 1, Arr, ObjParam)
            Return Arr
        Catch ex As Exception
            Throw
        End Try
    ...
    Public Function LoadIssueTrackerDetails(ByVal sAC As String, ByVal iACID As Integer, ByVal iYearID As Integer, ByVal iITPkID As Integer, ByVal iAuditID As Integer) As DataTable
        Dim sSql As String
        Try
            sSql = "Select *,MIM_Name,MIM_Color from CAIQ_IssueTracker_details "
    ...
    Public Function GetIssueTrackerSelectedStatus(ByVal sAC As String, ByVal iACID As Integer, ByVal iAuditID As Integer, ByVal iITID As Integer) As String
        Dim sSql As String
        Try
            sSql = "Select CID_Status From CAIQ_IssueTracker_details Where CID_AuditCodeID=" & iAuditID & " And CID_PKID=" & iITID & " And CID_CompID=" & iACID & ""
    ...
    Public Function LoadExistingFollowup(ByVal sNameSpace As String, ByVal iACID As Integer, ByVal iYearID As Integer, ByVal iAuditID As Integer, ByVal iIssueStatus As Integer) As DataTable
        ...
        Try
            ...
            sSql = sSql & " b.AUD_Title,b.AUD_MonthID,c.CUST_ID,c.CUST_NAME,d.MIM_ID, d.MIM_Color,d.MIM_Name, e.CRAD_PKID from CAIQ_IssueTracker_details a "
            sSql = sSql & " left join Audit_Schedule b on b.AUD_ID=a.CID_AuditCodeID and b.AUD_YearID=" & iYearID & " "
            ...
            sSql = sSql & " left join CRPA_ChecklistAuditAssest e on e.CRAD_PKID=a.CID_ChecklistID"
    ...
    Public Function LoadIssueFollowupFromPKID(ByVal sNameSpace As String, ByVal iACID As Integer, ByVal iYearID As Integer, ByVal iPKID As Integer) As DataTable
        ...
        Try
            ...
            sSql = sSql & " d.cap_id as processID,d.CAP_PROCESSNAME as Processname,e.CASP_ID as SubProcessID,e.CASP_SUBPROCESSNAME as SubProcess, f.CF_PKID as FactorID, f.CF_Name as FactorName, "
            sSql = sSql & " g.CFC_PKID as FCategoryID, g.CFC_Name as FCategoryName, h.CD_PKID as DescriptorsID, h.CD_Name as DescriptorName, i.CCD_PKID as DescriptionID, i.CCD_Name as DescName, "
            sSql = sSql & " a.CRAD_Results, a.CRAD_COMMENTS,k.*, l.MIM_Color, l.MIM_Name from CRPA_ChecklistAuditAssest a "
            sSql = sSql & " Left Join CRPA_Section b on b.CAS_Id=a.CRAD_SECTIONID "
            sSql = sSql & " Left Join CRPA_SubSection c on c.CASU_Id=a.CRAD_SUBSECTIONID"
            sSql = sSql & " Left Join CRPA_Process d on d.CAP_Id=a.CRAD_ProcessID"
            sSql = sSql & " Left Join CRPA_SubProcess e on e.CASP_ID=a.CRAD_SUBPROCESSID"
            ...
            sSql = sSql & " Left Join CAIQ_Descriptors h on h.CD_PKID=a.CRAD_DescriptorID"
            sSql = sSql & " Left Join CAIQ_CategoryDescription i on i.CCD_PKID=a.CRAD_DescriptionID"
            sSql = sSql & " Left Join CAIQ_IssueTracker_details k on k.CID_ChecklistID=a.CRAD_PKID"
    ...
    Public Sub SavedIssueTracker(ByVal sAC As String, ByVal sACID As String, ByVal iYearID As Integer, ByVal iUserID As Integer, ByVal iITID As Integer)
        Dim sSql As String
        Try
            sSql = "Update CAIQ_IssueTracker_details Set CID_Status='Saved',CID_SubmittedBy=" & iUserID & ",CID_SubmittedOn=GetDate() Where CID_PKID=" & iITID & "  And CID_CompID='" & sACID & "'"
            objDBL.SQLExecuteNonQuery(sAC, sSql)
        Catch ex As Exception
            Throw
        End Try
    ...
    Public Function SaveIssueTrackerHistory(ByVal sAC As String, ByVal objclsIssueFollowupTracker As clsIssueFollowupTracker, ByVal iCIDPKID As Integer)
        ...
        Try
            ...
            ObjParam(iParamCount).Value = objclsIssueFollowupTracker.iCIH_PKID
            ObjParam(iParamCount).Direction = ParameterDirection.Input
            iParamCount += 1
            ...
            Arr(0) = "@iUpdateOrSave"
            Arr(1) = "@iOper"
            Arr = objDBL.ExecuteSPForInsertARR(sAC, "spCAIQ_IssueTracker_History", 1, Arr, ObjParam)
            Return Arr
        Catch ex As Exception
            Throw
        End Try
    ...
    Public Function LoadActionPlanGrid(ByVal sAC As String, ByVal iACID As Integer, ByVal iITID As Integer) As DataTable
        ...
        Try
            ...
            sSql = sSql & " CIH_FunctionManagerID,a.usr_FullName as aUsrName,b.usr_FullName From CAIQ_IssueTracker_History"
