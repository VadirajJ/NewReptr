...
Public Structure strCRPA_AuditAssest
...
Public Class ClsAuditScore
    ...
    Public Function LoadSectiondetails(ByVal sAC As String, ByVal iACID As Integer) As DataTable
        ...
        End Try
    End Function
    Public Function LoadControlDomain(ByVal sAC As String, ByVal iACID As Integer) As DataTable
        Dim sSql As String
        Dim dt As New DataTable
        Try
            sSql = "select CASU_ID as IDText,concat(CASU_Code,'-',CASU_SUBSECTIONNAME) as NameText from CRPA_SubSection where CASU_SECTIONID=5 and CASU_DELFLG='A' and CASU_CompId=" & iACID & "  order by CASU_ID"
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function LoadControlSpecification(ByVal sAC As String, ByVal iACID As Integer, ByVal iSubSecID As Integer) As DataTable
        Dim sSql As String
        Dim dt As New DataTable
        Try
            sSql = "select CAP_ID as IDText,concat(CAP_code,'-',CAP_PROCESSNAME) as NameText from CRPA_Process where CAP_SECTIONID=5 and "
            sSql = sSql & " CAP_SUBSECTIONID=" & iSubSecID & " and CAP_DELFLG='A' and CAP_CompId=" & iACID & "  order by CAP_ID"
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function LoadCAQuestions(ByVal sAC As String, ByVal iACID As Integer, ByVal iSubSecID As Integer, ByVal iProID As Integer) As DataTable
        Dim sSql As String
        Dim dt As New DataTable
        Try
            sSql = "select CASP_ID as IDText,concat(CASP_code,'-',CASP_SUBPROCESSNAME) as NameText from CRPA_SubProcess where CASP_SECTIONID=5  "
            sSql = sSql & " and CASP_SUBSECTIONID=" & iSubSecID & " and CASP_PROCESSID=" & iProID & " and CASP_DELFLG='A' and CASP_CompId=" & iACID & "  order by CASP_ID"
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function LoadFactors(ByVal sAC As String, ByVal iACID As Integer) As DataTable
        Dim sSql As String
        Dim dt As New DataTable
        Try
            sSql = "select CF_PKID,CF_Name from CAIQ_Factors where CF_AuditID=5 and CF_STATUS='A' and CF_CompId=" & iACID & "  order by CF_PKID"
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function LoadFactorCategory(ByVal sAC As String, ByVal iACID As Integer, ByVal iFactorID As Integer) As DataTable
        Dim sSql As String
        Dim dt As New DataTable
        Try
            sSql = "select CFC_PKID, CFC_Name from CAIQ_FactorCategory where CFC_AuditID=5 and CFC_FactorID=" & iFactorID & " and CFC_STATUS='A' and CFC_CompId=" & iACID & " order by CFC_PKID"
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function LoadDescriptors(ByVal sAC As String, ByVal iACID As Integer) As DataTable
        Dim sSql As String
        Dim dt As New DataTable
        Try
            sSql = "select CD_PKID, CD_Name from CAIQ_Descriptors where CD_AuditID=5 and CD_STATUS='A' and CD_CompId=" & iACID & " order by CD_PKID"
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function GetDescriptionDetails(ByVal sAC As String, ByVal iACID As Integer, ByVal iFactorID As Integer, ByVal iCatId As Integer, ByVal iDescID As Integer) As DataTable
        Dim sSql As String
        Dim dt As New DataTable
        Try
            sSql = "select CCD_PKID as IDText ,Concat(CCD_DescValue,'-',CCD_Name) as NameText from CAIQ_CategoryDescription where CCD_AuditID=5 and CCD_FactorID=" & iFactorID & " and CCD_CategoryID=" & iCatId & " "
            sSql = sSql & "and CCD_DescriptorID=" & iDescID & " and CCD_STATUS='A' and CCD_CompID=" & iACID & ""
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function GetAUDRatingPoint(ByVal sAC As String, ByVal iACID As Integer, ByVal iFacID As Integer, ByVal iCatID As Integer, ByVal iDescID As Integer) As Integer
        Dim sSql As String
        Dim iRet As New Integer
        Try
            sSql = "select CCD_DescValue from CAIQ_CategoryDescription where CCD_FactorID=" & iFacID & " and CCD_CategoryID=" & iCatID & " and CCD_DescriptorID=" & iDescID & " "
            sSql = sSql & "and CCD_AuditID=5 and CCD_FLAG='A' and CCD_CompId=" & iACID & ""
        ...
        End Try
    End Function
    Public Function GetAUDSectionScore(ByVal sAC As String, ByVal iACID As Integer, ByVal iAudID As Integer) As Integer
        Dim sSql As String, aSql As String
        Dim iRet As Integer = 0, iResults As Integer = 0, iCount As Integer = 0
        Dim dt As New DataTable
        Try
            sSql = "select distinct(CRAD_SUBSECTIONID) from CRPA_ChecklistAuditAssest where CRAD_CAuditID=" & iAudID & " and CRAD_compid=" & iACID & ""
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    aSql = "select min(CRAD_results) as CRAD_results  from CRPA_ChecklistAuditAssest where CRAD_CAuditID=" & iAudID & " and CRAD_SUBSECTIONID=" & dt.Rows(i).Item("CRAD_SUBSECTIONID") & ""
                    iRet = objDBL.SQLExecuteScalar(sAC, aSql)
                    iResults = iResults + iRet
                    iCount = iCount + 1
                Next
                iResults = (iResults / iCount)
            End If
    ...
    Public Function LoadScheduleDts(ByVal sAC As String, ByVal iACID As Integer, ByVal iSecID As Integer, ByVal iKitchenID As Integer, ByVal iMonthID As Integer, ByVal iYearID As Integer) As DataTable
        ...
        Dim dt As New DataTable
        Try
            sSql = "select * from Audit_Schedule where aud_sectionid='" & iSecID & "' and AUD_KitchenID='" & iKitchenID & "' and AUD_MonthID='" & iMonthID & "' and AUD_YearID='" & iYearID & "' and AUD_CompID='" & iACID & "' "
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
    ...
    Public Function CheckForData(ByVal sAC As String, ByVal iACID As Integer, ByVal iCusID As Integer, ByVal iSectionID As Integer, ByVal iYearID As Integer, ByVal iMonthId As Integer, ByVal iAUDSchId As Integer) As DataTable
        ...
        Try
            sSql = "select ca_pkid,CA_Status,CA_NETSCORE,CA_NAME_OF_OPS_HEAD,CA_ADDRESS,CA_NAME_OF_UNIT_PRESIDENT,CA_AUDITORNAME,CA_Date,AUD_MonthId from CRPA_AuditAssest  "
            sSql = sSql & " Left join Audit_Schedule b on b.aud_id=ca_asgno "
    ...
    '        sSql = sSql & " CA_AUDITORNAME,CA_Status from CRPA_AuditAssest where CA_LOCATIONID='" & iCusID & "' and "
    ...
    Public Function GetSubProcessName(ByVal sAC As String, ByVal iACID As Integer, ByVal iAUDCheckID As Integer, ByVal iAuditID As Integer) As DataTable
        ...
        End Try
    End Function
    Public Function GetCAIQFactors(ByVal sAC As String, ByVal iACID As Integer) As DataTable
        Dim sSql As String : Dim dt As New DataTable
        Try
            sSql = "select CF_PKID as IDText, CF_Name As NameText from CAIQ_Factors where CF_AuditID=5 and CF_CompId=" & iACID & " and CF_FLAG='A' order by IDText"
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function GetCAIQDescriptors(ByVal sAC As String, ByVal iACID As Integer) As DataTable
        Dim sSql As String : Dim dt As New DataTable
        Try
            sSql = " select CD_Pkid as IDText, CD_Name As NameText from CAIQ_Descriptors where CD_AuditID=5 and CD_CompId=1 and CD_FLAG='A' order by IDText"
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function GetListOfDescription(ByVal sAC As String, ByVal iACID As Integer, ByVal iFacID As Integer, ByVal iDescID As Integer) As DataTable
        Dim sSql As String : Dim dt As New DataTable
        Try
            sSql = " select CCD_PKID as IDText,concat(CCD_DescValue,' - ',CCD_Name) As NameText from CAIQ_CategoryDescription where CCD_AuditID=5 and CCD_FactorID=" & iFacID & " and CCD_DescriptorID=" & iDescID & " "
            sSql = sSql & " and CCD_CompId=" & iACID & " and CCD_FLAG='A' order by CCD_PKID"
            dt = objDBL.SQLExecuteDataTable(sAC, sSql)
            Return dt
        Catch ex As Exception
            Throw
    ...
    Public Function GetKitchenID(ByVal sAC As String, ByVal iACID As Integer, ByVal iAudID As Integer, ByVal iYearID As Integer, ByVal iMonthId As Integer) As Integer
        ...
        Dim iRet As New Integer
        Try
            sSql = "Select AUD_KitchenID from Audit_Schedule where AUD_ID=" & iAudID & " and AUD_MonthID=" & iMonthId & " and  AUD_YearID=" & iYearID & " and AUD_compID=" & iACID & ""
            iRet = objDBL.SQLExecuteScalarInt(sAC, sSql)
            Return iRet
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function LoadExisting(ByVal sNameSpace As String, ByVal iAuditID As Integer, ByVal iACID As Integer) As DataTable
        ...
        dtDisplay.Columns.Add("Factors")
        dtDisplay.Columns.Add("FactorsID")
        dtDisplay.Columns.Add("Descriptors")
        dtDisplay.Columns.Add("DescriptorsID")
        dtDisplay.Columns.Add("FDescription")
        dtDisplay.Columns.Add("FDescriptionID")
        dtDisplay.Columns.Add("Result")
        dtDisplay.Columns.Add("AuditComment")
        dtDisplay.Columns.Add("AUDObservation")
        ...
        Try
            ...
            sSql = sSql & " a.CRAD_Results, a.CRAD_COMMENTS,a.CRAD_AUDObservation,a.CRAD_CREATEDBY, a.CRAD_CREATEDON from CRPA_ChecklistAuditAssest a "
            ...
            sSql = sSql & " where CRAD_CAuditID ='" & iAuditID & "' and CASP_CompId ='" & iACID & "' order by CRAD_PKID asc"
            ...
            If dr.HasRows Then
                While dr.Read
                    ...
                    End If
                    If IsDBNull(dr("FactorName")) = False Then
                        dRow("Factors") = dr("FactorName")
                    End If
                    If IsDBNull(dr("FactorID")) = False Then
                        dRow("FactorsID") = dr("FactorID")
                    End If
                    If IsDBNull(dr("DescriptorName")) = False Then
                        dRow("Descriptors") = dr("DescriptorName")
                    End If
                    If IsDBNull(dr("DescriptorsID")) = False Then
                        dRow("DescriptorsID") = dr("DescriptorsID")
                    End If
                    If IsDBNull(dr("DescName")) = False Then
                        dRow("FDescription") = dr("DescName")
                    End If
                    If IsDBNull(dr("DescriptionID")) = False Then
                        dRow("FDescriptionID") = dr("DescriptionID")
                    End If
                    If IsDBNull(dr("CRAD_Results")) = False Then
                        dRow("Result") = dr("CRAD_Results")
                    End If
                    If IsDBNull(dr("CRAD_COMMENTS")) = False Then
                        dRow("AuditComment") = dr("CRAD_COMMENTS")
                    End If
                    If IsDBNull(dr("CRAD_AUDObservation")) = False Then
                        dRow("AUDObservation") = dr("CRAD_AUDObservation")
                        If IsDBNull(dr("CRAD_CREATEDBY")) = False Then
                            dRow("CreatedBy") = GetAuditorsUserName(sNameSpace, iACID, dr("CRAD_CREATEDBY"))
                        End If
                        If IsDBNull(dr("CRAD_CREATEDON")) = False Then
                            dRow("CreatedOn") = dr("CRAD_CREATEDON").ToString.Substring(0, 10)
                        End If
                    End If
        ...
        End Try
    End Function
    Public Function GetAuditorsUserName(ByVal sAC As String, ByVal iACID As Integer, ByVal UserID As Integer)
        Dim sSql As String
        Dim dt As New DataTable
        ...
        Try
            ...
            If UserID > 0 Then
                sSql = "select USR_FullName from sad_userdetails where USR_id =" & UserID & " and USR_dutyStatus='A' and USR_compid=" & iACID & ""
                UserName = objDBL.SQLExecuteScalar(sAC, sSql)
            End If
    ...
    Public Function SaceAuditDetails(ByVal sAC As String, ByVal iACID As Integer, ByVal SArray As Array) As Array
        ...
        End Try
    End Function
    Public Function SaveAuditAssest(ByVal sAC As String, ByVal objAuditAssest As strCRPA_AuditAssest) As Array
        ...
        Try
            ...
            ObjSFParam(iRCSAParamCount).Value = ""
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1
            ...
            Arr = objDBL.ExecuteSPForInsertARR(sAC, "spCRPA_AUDITASSEST", 1, Arr, ObjSFParam)
            Return Arr
        Catch ex As Exception
            Throw
        End Try
    ...
    '        iMaxID = objDBL.SQLExecuteScalarInt(sAC, "Select Count(*)+1 from CRPA_AuditAssest where ca_financialyear=" & iYearID & "")
    '        sModuleCode = "GMP"
    ...
    Public Function LoadGridDetails2(ByVal sNameSpace As String, ByVal iSectionID As Integer, ByVal iACID As Integer) As DataTable
        ...
        End Try
    End Function
    Public Function SaveCheckListAudit(ByVal sAC As String, ByVal iPkID As Integer, ByVal iAssignID As Integer, ByVal iLocationID As Integer, ByVal iMonthID As Integer, ByVal sdate As DateTime,
                                      ByVal yearid As String, ByVal iSectionId As Integer, ByVal iSubSectionId As Integer, ByVal iProcessId As Integer, ByVal iSubProcessId As Integer, ByVal iFactorID As Integer,
                                      ByVal iFCategoryID As Integer, ByVal iDescriptorID As Integer, ByVal iDescriptionID As Integer, ByVal iScoreResults As Integer, ByVal scomments As String,
                                      ByVal createdby As String, ByVal updatedby As String, ByVal ip As String, ByVal AUDObsrv As String, ByVal compid As String) As Array
        Dim ObjSFParam() As OleDb.OleDbParameter = New OleDb.OleDbParameter(22) {}
        Dim iRCSAParamCount As Integer
        Dim Arr(1) As String
        Try
            iRCSAParamCount = 0
            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_PKID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iPkID
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_CAuditID", OleDb.OleDbType.VarChar, 50)
            ObjSFParam(iRCSAParamCount).Value = iAssignID
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_LOCATIONID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iLocationID
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_MonthID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iMonthID
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_DATE", OleDb.OleDbType.Date)
            ObjSFParam(iRCSAParamCount).Value = sdate
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_YEARID ", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = yearid
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_SECTIONID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iSectionId
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_SUBSECTIONID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iSubSectionId
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_PROCESSID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iProcessId
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_SUBPROCESSID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iSubProcessId
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_FactorsID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iFactorID
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_FCategoryID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iFCategoryID
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_DescriptorID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iDescriptorID
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_DescriptionID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iDescriptionID
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_Results", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = iScoreResults
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_COMMENTS", OleDb.OleDbType.VarChar, 500)
            ObjSFParam(iRCSAParamCount).Value = scomments
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_CREATEDBY", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = createdby
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_UPDATEDBY", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = updatedby
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_IPAddress", OleDb.OleDbType.VarChar)
            ObjSFParam(iRCSAParamCount).Value = ip
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_CompID", OleDb.OleDbType.Integer)
            ObjSFParam(iRCSAParamCount).Value = compid
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1

            ObjSFParam(iRCSAParamCount) = New OleDb.OleDbParameter("@CRAD_AUDObservation", OleDb.OleDbType.VarChar)
            ObjSFParam(iRCSAParamCount).Value = AUDObsrv
            ObjSFParam(iRCSAParamCount).Direction = ParameterDirection.Input
            iRCSAParamCount += 1
            ...
            Arr = objDBL.ExecuteSPForInsertARR(sAC, "spChecklistAuditAssest", 1, Arr, ObjSFParam)
            Return Arr
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Function GetColorAndRange(ByVal sAC As String, ByVal iACID As Integer, ByVal iYearID As Integer, ByVal iValueID As Integer, ByVal iSecID As Integer) As DataTable
        Dim sSql As String : Dim dt As New DataTable
        Try
            sSql = "Select CRAT_Name,CRAT_Color From CRPA_Rating Where  CRAT_FLAG='A' And CRAT_STARTVALUE<=round(" & iValueID & ",1) AND CRAT_ENDVALUE>=round(" & iValueID & ",1) And CRAT_COMPID=" & iACID & ""
            If iSecID > 0 Then
    ...
    Public Function UpdateNetScore(ByVal sAC As String, ByVal iACID As Integer, ByVal iNetscore As Double, ByVal iPkid As Integer) As String
        Dim sSql As String = ""
        Try
            sSql = "update CRPA_AuditAssest set CA_NetScore ='" & iNetscore & "' where CA_PKID='" & iPkid & "'"
            objDBL.SQLExecuteNonQuery(sAC, sSql)
        Catch ex As Exception
            Throw
        End Try
    ...
    Public Function Updatestatus(ByVal sAC As String, ByVal iACID As Integer, ByVal iPkid As Integer) As String
        Dim sSql As String = ""
        Try
            sSql = "update CRPA_AuditAssest set CA_Status ='Submitted' where CA_PKID='" & iPkid & "'"
            objDBL.SQLExecuteNonQuery(sAC, sSql)
        Catch ex As Exception
            Throw
        End Try
    ...
    Public Function UpdateScheduleStatus(ByVal sAC As String, ByVal iACID As Integer, ByVal iSecID As Integer, ByVal iKitchenID As Integer, ByVal iMonthID As Integer, ByVal iYearID As Integer, ByVal sStatus As String) As String
        ...
        Try
            If sStatus = "Saved" Then
                sSql = "Update Audit_Schedule set AUD_Operation ='Saved' "
            ElseIf sStatus = "Submitted" Then
                sSql = "Update Audit_Schedule set AUD_Operation ='Submitted' "
            End If
    ...
    Public Function CheckForExistingCombo(ByVal sAC As String, ByVal iACID As Integer, ByVal iSubProID As Integer, ByVal iFactorID As Integer, ByVal iRiskAssessID As Integer,
                                          ByVal iDescriptorID As Integer, ByVal iAUDID As Integer) As Boolean
        Dim sSql As String = ""
        Dim iCount As Integer = 0
        Try
            sSql = "select count(CRAD_PKID) from CRPA_ChecklistAuditAssest where CRAD_SECTIONID=5 and CRAD_CAuditID=" & iAUDID & " and CRAD_SUBProcessID=" & iSubProID & " "
            sSql = sSql & " and CRAD_FactorsID=" & iFactorID & " and CRAD_FCategoryID=" & iRiskAssessID & " and CRAD_DescriptorID=" & iDescriptorID & " and CRAD_CompID=" & iACID & ""
